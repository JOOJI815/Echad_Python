...
with st.form("booking_form"):
    ...
    name = st.text_input("신청자 이름", value="")
    number = st.text_input("전화번호", value="")
...
st.link_button("🌐 공식 예약 페이지 열기", "http://www.scdaedeok.or.kr/arena_booking.html?arenaId=SF0.1",
...
if submit:
    ...
    now = datetime.now()
    # 다음달 1일 구하기
    next_month_start = (now.replace(day=28) + timedelta(days=10)).replace(day=1)
    ...
    for i in range(35):
        ...
        if py_day == day_idx:
            # 가격 계산 로직 (C# 코드 참고)
            hour_rate = default_pay if check_date.weekday() >= 5 else (default_pay / 2)
            count = 2 if option_2h else 1
            current_total = (hour_rate + (default_light_pay if option_light else 0)) * count
